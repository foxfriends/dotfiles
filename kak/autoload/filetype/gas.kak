...
provide-module gas %{
    ...
    # Labels
    add-highlighter shared/gas/code/ regex ^\h*([A-Za-z0-9_.\-#@~]+): 0:meta
    ...
    # Other directives?
    add-highlighter shared/gas/code/ regex ((^|\s+)(global|extern|section|d[db])(\h+|$)) 0:function
    ...
    # Registers
    add-highlighter shared/gas/code/ regex \%?\b(([re](ax|bx|cx|dx|si|di|bp|sp))|(al|bl|cl|dl|sil|dil|bpl|spl)|(r[8-9][dwb])|(r1[0-5][dwb])|(cs|ds|es|fs|gs|ss|ip|eflags)|([xy]mm[0-9]|[xy]mm1[0-5]))\b 0:variable
    # Memory location
    add-highlighter shared/gas/code/ regex (\[|\]) 0:variable

    # Size directives
    add-highlighter shared/gas/code/ regex \b(dword)\b 1:keyword
