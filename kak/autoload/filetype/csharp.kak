...
add-highlighter shared/csharp/code/ regex "\b(while|for|foreach|in|out|if|else|do|static|readonly|delegate|event|switch|case|default|class|struct|record|interface|enum|break|continue|return|async|await|using|namespace|try|catch|throw|new|extends|implements|throws|instanceof|finally|as|get|init|set|var|yield|const|is|not|with|when|where)\b" 0:keyword
add-highlighter shared/csharp/code/ regex "\b(final|public|protected|private|partial|abstract|sealed|internal|virtual|override|file|required)\b" 0:attribute
