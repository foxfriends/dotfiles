...
evaluate-commands %sh{
    for ft in c cpp objc; do
        ...
        cat <<-EOF
            ...
            add-highlighter shared/$ft/documentation_comment region /\*(\*[^/]|!) \*/ ref doc_comment
            add-highlighter shared/$ft/line_documentation_comment region //[/!] $ ref doc_comment
            add-highlighter shared/$ft/comment region /\\* \\*/ ref comment
            ...
            add-highlighter shared/$ft/code/function     regex \b([A-Za-z_][a-zA-Z_0-9]*)\s*(?=\() 1:function
            add-highlighter shared/$ft/code/modulepath   regex \b([A-Za-z_][a-zA-Z_0-9]*):: 1:module
            add-highlighter shared/$ft/code/operator     regex (\[|\]|=|==|!=|\+=|-=|\*=|/=|%=|<|>|<=|>=|=>|->|\+|-|/|\*|%|~|\||\|\||&|&&|!|\^|\?|<<|>>|<<=|>>=|\.|\.\.|\.\.=) 1:operator
EOF
    done
}
