...
provide-module trilogy %§
    ...
    add-highlighter shared/trilogy/code/    regex \b(and|assert|as|at|become|break|cancel|const|continue|case|defer|do|else|end|exit|export|extern|fn|for|func|if|in|is|let|match|module|mut|not|or|pass|proc|qy|resume|return|rule|super|test|then|typeof|use|when|while|with|yield)\b 1:keyword
    add-highlighter shared/trilogy/code/    regex \b(async|await|catch|class|data|enum|except|extends|implements|import|inline|instanceof|interface|lazy|lens|loop|macro|next|oper|prec|protocol|static|struct|switch|tag|trait|try|type|unless|until|where)\b 1:error
